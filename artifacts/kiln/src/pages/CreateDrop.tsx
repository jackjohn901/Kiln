...
import { ChevronLeft, Zap, Check, Loader2, X, ImageIcon } from "lucide-react";
import Nav from "@/components/Nav";
import { useUpload } from "@/hooks/useUpload";

const TECHNIQUES = [
  "Glass Blowing", "Kiln Forming", "Lost-Wax Cast", "Flameworking",
  "Metal Forging", "Welding", "Bronze Casting", "Stone Carving",
  "Ceramics", "Fiber Arts", "Mixed Media",
];
...
export default function CreateDrop() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.title || !form.price || !form.dropDate) {
      setError("Title, price, and drop date are required.");
      return;
    }
    setError("");
    setSubmitting(true);
    try {
      ...
      const res = await fetch("/api/drops", {
        ...
        body: JSON.stringify({
          ...
          dropDate: new Date(form.dropDate).toISOString(),
          imageUrl: imageUrl || null,
          isPatronEarlyAccess: form.isPatronEarlyAccess,
          tags: form.tags,
        }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({})) as { error?: string };
        throw new Error(d.error ?? "Failed to create drop");
      }
      setDone(true);
    } catch (err) {
  ...
  if (done) {
    return (
      <div className="min-h-screen bg-[#12100e] flex flex-col items-center justify-center gap-6 p-8 text-center">
        ...
        <h2 className="font-serif text-2xl text-amber-100">Drop Scheduled</h2>
        <p className="text-stone-400 max-w-sm">Your drop is live. Collectors can join the waitlist and be notified when it goes live.</p>
        <div className="flex gap-3">
          ...
          <button
            onClick={() => { setDone(false); setForm({ title: "", description: "", technique: "", price: "", edition: "1", dropDate: "", imageUrl: "", isPatronEarlyAccess: false, tags: [] }); setImagePreview(""); setImageFile(null); }}
  ...
  return (
    <div className="min-h-screen bg-[#12100e]">
      ...
      <div className="mx-auto max-w-xl px-4 py-10 pb-28 md:pb-10">
        <button
          onClick={() => (window.history.length > 1 ? window.history.back() : navigate("/drops"))}
        ...
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-serif text-2xl text-amber-100">Schedule a Drop</h1>
            <p className="text-sm text-stone-500">Release a limited piece with a countdown</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Image */}
          <div>
            <label className="mb-2 block text-sm font-medium text-stone-400">Drop Image</label>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Amber Series — Edition 3"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-stone-400">Edition Size</label>
              <input
                type="number"
                min="1"
                value={form.edition}
          ...
          {/* Drop Date */}
          <div>
            <label className="mb-1.5 block text-sm font-medium text-stone-400">Drop Date & Time <span className="text-rose-400">*</span></label>
            <input
              type="datetime-local"
              value={form.dropDate}
          ...
          {/* Patron early access */}
          <label className="flex items-center gap-3 cursor-pointer rounded-xl border border-white/10 bg-stone-900/40 p-4 hover:border-amber-500/20 transition-colors">
            <input
              type="checkbox"
              checked={form.isPatronEarlyAccess}
              onChange={(e) => set("isPatronEarlyAccess", e.target.checked)}
              className="h-4 w-4 accent-amber-500"
            />
            <div>
              <p className="text-sm font-medium text-stone-200">Patron early access</p>
              <p className="text-xs text-stone-500">Patrons get 24h early access before the public drop</p>
            </div>
          </label>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="glass, vessel, limited..."
          ...
          >
            {submitting || uploading ? (
              <>
                ...
                {uploading ? "Uploading…" : "Scheduling…"}
              </>
            ) : (
              "Schedule Drop"
            )}

...
export function CharcoalBackground() {
  const [enabled, setEnabled] = useState(true);
  const [bg, setBg] = useState<"white" | "charcoal">("charcoal");
  ...
  return (
    <div className="min-h-screen bg-[#12100e] flex items-center justify-center p-6">
      <div className="w-[360px] space-y-4">
        ...
        {/* Image Preview Card */}
        <div
          className={`relative w-full rounded-2xl overflow-hidden transition-all duration-300 ${bgClass}`}
        ...
        >
          <img
            ...
            style={enabled && bg === "charcoal" ? { mixBlendMode: "luminosity", opacity: 0.92 } : {}}
          />

          {/* Soft shadow beneath piece */}
          {enabled && (
            <div
              className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4/5 h-10 rounded-full pointer-events-none"
              style={{
          ...
          {/* Subtle inner shadow for depth on charcoal */}
          {enabled && bg === "charcoal" && (
            <div
